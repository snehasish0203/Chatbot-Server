  ...
  return (
    <div className="flex h-screen bg-gray-900 text-white">
              ...
              messages && messages.length>0 ? messages.map((e,i)=>(
                <div key={i}>
                  <div className="mb-4 p-4 rounded bg-blue-700 text-white">
                    <div className="bg-white p-2 rounded-full text-black text-2x h-10">
                      <CgProfile/>
                    </div>
                    {e.question}
                  </div>
     ...
     {chats && chats.length===0 ? (
        ...
        <form onSubmit={submitHandler} className="flex justify-center item-center">

          <input type="text"
            className="flex-grow p-4 bg-gray-700 rounded-1 text-white outline-none" 
            placeholder="Enter a promp here" 
            value={prompt} 
            onChange={e=>setPrompt(e.target.value)} 
            required/>
            <button className="p-4 bg-gray-700 rounded-r text-2x1 text-white">
            <IoMdSend />
            </button>
        </form>
      </div>
